...
sources.forEach (source)->
  fetch source, (result)->
    console.log "+ #{source.title.replace(/[^\w\u4e00-\u9fa5]+/g, '_').replace(/^_+|_+$/g, '').replace(/[\u4e00-\u9fa5]+/g, (word)->pinyin(word, segment: true, style: pinyin.STYLE_NORMAL).join('_')).toLowerCase()}"
    ...
    for target in result
      unless source.excludes and target.host in source.excludes
        console.log "++ #{target.title.replace(/[^\w\u4e00-\u9fa5]+/g, '_').replace(/^_+|_+$/g, '').replace(/[\u4e00-\u9fa5]+/g, (word)->pinyin(word, segment: true, style: pinyin.STYLE_NORMAL).join('_')).toLowerCase()}"
